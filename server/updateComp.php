...
// DELETE from component_images WHERE component_id = $jsonComp->component_id AND image not in ($jsonComp->component_imgs)
//INSERT todas las imgs del array
for ($i = 0; $i < count($jsonComp->component_imgs); $i++) {
	$sentence2 = $bd->prepare("REPLACE INTO component_images(component_id, image) VALUES (?,?)");
	$res2 = $sentence2->execute([$jsonComp->component_id, $jsonComp->component_imgs[$i]]);
}
