...
$sentence2 = $bd->prepare("DELETE FROM component_images WHERE component_id = ?");
$res2 = $sentence2->execute([$jsonComp->component_id]);
for ($i = 0; $i < count($jsonComp->component_imgs); $i++) {
	$sentence3 = $bd->prepare("INSERT INTO component_images(component_id, image) VALUES (?,?)");
